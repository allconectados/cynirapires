...
use Modules\Teacher\Entities\NotaSegundoBimestre;
...
class NotaSegundoBimestreController extends Controller
{
    ...
    public function index($year, $stage, $serie, $room, $discipline)
    {
        ...
        $studentsNote = DB::table('notas_segundo_bimestres')
        ...
        if ($studentsNote->isNotEmpty()) {
            $titlePage = 'NOTAS DO 2º BIMESTRE: '.$discipline->title.' / '.$room->title;
            $studentsNote = DB::table('notas_segundo_bimestres')
                ->where('ano', '=', $year->title)
                ->where('stage', '=', $stage->title)
                ->where('serie', '=', $serie->title)
                ...
                ->where('room', '=', $room->title)
                ->get();
            return view('teacher::notas.edit-notas-segundo-bimestre',
            ...
            return view('teacher::notas.create-notas-segundo-bimestre',
    ...
    public function store()
    {
        ...
        for ($i = 0; $i < count($ano); $i++) {
            ...
            $create = NotaSegundoBimestre::create($datasave);
            ...
            if ($create) {
                ...
                DB::table('notas_quinto_conceitos')
                    ...
                    ->where('number', $this->request->number[$i])->where('name',
                        ...
                        [
                            'nota_segundo_bimestre' => $this->request->nota[$i],
                            'faltas_segundo_bimestre' => $this->request->falta[$i] - $this->request->faltas_compensadas[$i],
                        ]
                    );
    ...
    {
        $student = NotaSegundoBimestre::find($id);
        ...
        foreach ($this->request->id as $item => $value) {
            ...
            $updateBimestre = NotaSegundoBimestre::where('id', $request->id[$item])->update($dataUpdate);
            if ($updateBimestre) {
                ...
                DB::table('notas_quinto_conceitos')->where('ano', $request->ano[$item])->where('stage',
                    ...
                    ->where('number', $request->number[$item])->where('name',
                        ...
                        [
                            'nota_segundo_bimestre' => $request->nota[$item],
                            'faltas_segundo_bimestre' => $request->falta[$item] - $request->faltas_compensadas[$item],
                        ]
                    );
