...
class NotaConceitoFinalController extends Controller
{
    ...
    public function index($year, $stage, $serie, $room, $discipline)
    {
        ...
        $studentsNote = DB::table('notas_conceito_final')
        ...
        if ($studentsNote->isNotEmpty()) {
            ...
            $studentsNote = DB::table('notas_conceito_final')
                ...
                ->where('room', '=', $room->title)
                ->get();
            return view('teacher::notas.edit-notas-conceito-final',
            ...
            $students = DB::table('students')
                ...
                ->where('room', '=', $room->title)
                ->get();
            $studentsNotePrimeiroBimestre = DB::table('notas_primeiro_bimestres')->select('id','number','name','status', 'nota_final',
                'total_de_faltas')
                ->where('ano', '=', $year->title)
                ->where('stage', '=', $stage->title)
                ->where('serie', '=', $serie->title)
            ...
            $studentsNoteSegundoBimestre = DB::table('notas_segundo_bimestres')->select('id','number','name','status', 'nota_final',
                'total_de_faltas')
                ->where('ano', '=', $year->title)
                ->where('stage', '=', $stage->title)
                ->where('serie', '=', $serie->title)
            ...
            $studentsNoteTerceiroBimestre = DB::table('notas_terceiro_bimestres')->select('id','number','name','status', 'nota_final',
                'total_de_faltas')
                ->where('ano', '=', $year->title)
                ->where('stage', '=', $stage->title)
                ->where('serie', '=', $serie->title)
            ...
            $studentsNoteQuartoBimestre = DB::table('notas_quarto_bimestres')->select('id', 'nota_final', 'total_de_faltas')
                ->where('ano', '=', $year->title)
                ->where('stage', '=', $stage->title)
                ->where('serie', '=', $serie->title)
                ->where('teacher', '=', auth()->user()->name)
            ...
            return view('teacher::notas.create-notas-conceito-final',
                compact('titlePage', 'year', 'stage', 'serie', 'room', 'discipline','students',
                    'studentsNotePrimeiroBimestre', 'studentsNoteSegundoBimestre', 'studentsNoteTerceiroBimestre',
                    'studentsNoteQuartoBimestre'));
        }
    }
    ...
    public function store()
    {
        ...
        for ($i = 0; $i < count($ano); $i++) {
            ...
            DB::table('notas_conceito_final')->insert($datasave);
        }
