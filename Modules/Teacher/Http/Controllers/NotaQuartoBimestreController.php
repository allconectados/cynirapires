...
class NotaQuartoBimestreController extends Controller
{
    ...
    public function index($year, $stage, $serie, $room, $discipline)
    {
        ...
        if ($studentsNote->isNotEmpty()) {
            $titlePage = 'NOTAS DO 4º BIMESTRE: '.$discipline->title.' / '.$room->title;
            $studentsNote = DB::table('notas_quarto_bimestres')
                ->where('ano', '=', $year->title)
                ->where('stage', '=', $stage->title)
                ->where('serie', '=', $serie->title)
    ...
    public function store()
    {
        ...
        for ($i = 0; $i < count($ano); $i++) {
            ...
            $create = NotaQuartoBimestre::create($datasave);
            ...
            if ($create) {
                ...
                DB::table('notas_quinto_conceitos')
                    ...
                    ->where('number', $this->request->number[$i])->where('name',
                        ...
                        [
                            'nota_quarto_bimestre' => $this->request->nota[$i],
                            'faltas_quarto_bimestre' => $this->request->falta[$i] - $this->request->faltas_compensadas[$i],
                        ]
                    );
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($this->request->id as $item => $value) {
            ...
            $updateBimestre = NotaQuartoBimestre::where('id', $request->id[$item])->update($dataUpdate);
            if ($updateBimestre) {
                ...
                DB::table('notas_quinto_conceitos')->where('ano', $request->ano[$item])->where('stage',
                    ...
                    ->where('number', $request->number[$item])->where('name',
                        ...
                        [
                            'nota_quarto_bimestre' => $request->nota[$item],
                            'faltas_quarto_bimestre' => $request->falta[$item] - $request->faltas_compensadas[$item],
                        ]
                    );
